...
<?php
	...
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		...
		$media_query	= "SELECT mb_media, COUNT( mb_media ) media_cnt FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' GROUP BY mb_media";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			$pc_query		= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_gubun='MOBILE'";
